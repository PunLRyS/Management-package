...
export default function Summary() {
  ...
  const handleSubmit = (e) => {
    ...
    const exportedPrice = parseInt(price); // Chuyển đổi giá xuất sang số nguyên
  
    if (isNaN(exportedQuantity)|| exportedQuantity <= 0 || exportedPrice <= 0 || isNaN(exportedPrice)) {
...
const currentPost = posts.find(post => post.id === selectedItem.id);
  ...
  const handleEdit = (item) => {
    ...
    setPrice(item.giaNhap); // Đặt giá hiện có vào input
    setIsOpen(true); // Mở popup
    ...
    const receiptCode = generateRandomCode(4);
      ...
      const receiptData = {
        ...
        danhSachHangHoa: exportedItems.map(item => ({
          ...
          giaNhap: item.giaNhap // Giữ nguyên trường giaNhap
        })),
      };
